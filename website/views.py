...
from .models import Chapter, Question, Quiz, Score, Subject, User
...
def add_subject():
    if request.method == "POST":
        ...
        if Subject.query.filter(db.func.lower(Subject.name) == name).first():
            ...
            try:
                new_subject = Subject(
                    name=name, description=description, qualification=qualification
                )
...
def view_chapters(subject_id):
    ...
    return render_template(
        "subjects_chapters/view_chapters.html", subject=subject, chapters=chapters
    )
...
def add_quiz(chapter_id):
    ...
    if request.method == "POST":
        try:
            ...
            new_quiz = Quiz(
                chapter_id=chapter.id, time_duration=time_duration, remarks=remarks
            )
...
def view_quizzes(chapter_id):
    ...
    return render_template(
        "quizzes/view_quizzes.html", chapter=chapter, quizzes=quizzes
    )
...
@views.route(
    "/admin/edit_question/<int:quiz_id>/<int:question_id>", methods=["GET", "POST"]
)
@login_required
@admin_required
...
def admin_analytics_data():
    ...
    subject_performance = (
        db.session.query(Subject.name, func.coalesce(func.avg(Score.total_score), 0))
        .select_from(Score)
    ...
    performance_distribution = (
        db.session.query(Score.total_score, func.count(Score.id))
        .join(User)
        .filter(User.is_admin == False)
        .group_by(Score.total_score)
        .all()
    )
    ...
    # New data for quizzes taken over time
    quizzes_over_time_data = (
        db.session.query(
    ...
    # New data for most used subjects
    most_used_subjects_data = (
    ...
    most_used_subjects = [
        ...
        for subject, attempts in most_used_subjects_data
    ]

    # New data for quiz results distribution (pass/fail/average)
    # For simplicity, let's define:
    # - Pass: Score >= 70%
    # - Average: 40% <= Score < 70%
    # - Fail: Score < 40%

    # First, get the total number of questions per quiz
    quiz_question_counts = (
        db.session.query(Quiz.id, func.count(Question.id).label("question_count"))
        .join(Question)
        .group_by(Quiz.id)
        .all()
    )
    ...
    quiz_questions = {quiz_id: count for quiz_id, count in quiz_question_counts}

    # Now analyze each score to determine pass/fail/average
    pass_count = 0
    fail_count = 0
    avg_count = 0

    for score in Score.query.all():
        # Skip if we don't know the total questions for this quiz
        if score.quiz_id not in quiz_questions or quiz_questions[score.quiz_id] == 0:
            continue

        # Calculate percentage
        percentage = (score.total_score / quiz_questions[score.quiz_id]) * 100
    ...
    user_performance_data = (
        ...
        .join(Score)
        .filter(User.is_admin == False)
        .group_by(User.id, User.full_name, User.email, User.qualification)
        .all()
    )
    ...
    user_performance = [
        ...
        for name, email, qualification, avg_score, quiz_attempts in user_performance_data
    ]
    ...
    return jsonify(
        {
            ...
            "most_used_subjects": most_used_subjects,
            "quiz_results_distribution": quiz_results_distribution,
...
def toggle_publish(quiz_id):
    ...
    try:
        ...
        db.session.commit()
        flash(
            f"Quiz {'published' if quiz.published else 'unpublished'} successfully!",
            "success",
        )
    except Exception as e:
        db.session.rollback()
...
def start_quiz(quiz_id):
    ...
    if previous_attempt:
        flash(
            "You have already attempted this quiz. Multiple attempts are not allowed. Please contact the admin if you believe this is an error.",
            "warning",
        )
        return redirect(url_for("views.user_dashboard"))
    ...
    flash(
        "⚠️ Before you start, ensure a stable internet connection and avoid switching tabs. Any violations may auto-submit your quiz.",
        "info",
    )
...
def user_analytics_data():
    ...
    avg_score = (
        db.session.query(db.func.avg(Score.total_score))
    ...
    subject_performance = (
        db.session.query(Subject.name, db.func.avg(Score.total_score))
        .select_from(Score)
        .join(Quiz, Quiz.id == Score.quiz_id)
    ...
    past_performance_data = [
        {
            "timestamp": p.time_stamp_of_attempt.strftime("%Y-%m-%d %H:%M:%S"),
            "score": p.total_score,
        }
...
def submit_quiz():
    ...
    questions = Question.query.filter(
        Question.quiz_id == quiz_id, Question.id.in_(map(int, responses.keys()))
    ).all()
...
def view_performance(quiz_id):
    score = (
        Score.query.filter_by(user_id=current_user.id, quiz_id=quiz_id)
        .order_by(Score.time_stamp_of_attempt.desc())
        .first()
    )
    if not score:
    ...
    if request.method == "POST":
        ...
        if dob_str:
            try:
                ...
                flash(
                    "Invalid date format for Date of Birth. Please use YYYY-MM-DD.",
                    "error",
                )
...
def admin_change_password():
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            flash(
                f"An error occurred while changing admin password: {str(e)}", "danger"
            )
            return redirect(url_for("views.admin_change_password"))
...
def leaderboard():
    ...
    leaderboard_data = (
        db.session.query(
            ...
            User.full_name,
            User.email,
            func.sum(Score.total_score).label(
                "total_score"
            ),  # Use total_score from Score model
        ...
        for index, entry in enumerate(leaderboard_data):
            if entry.id == current_user.id:
                current_user_rank_info = {
                    "rank": index + 1,
                    "total_score": (
                        entry.total_score if entry.total_score is not None else 0
                    ),
                    "quiz_attempts": (
                        entry.quiz_attempts if entry.quiz_attempts is not None else 0
                    ),
                }
                break
